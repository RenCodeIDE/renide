...
export class CodeWindow extends BaseWindow implements ICodeWindow {
	...
	load(
		...
		// Load URL
		const htmlFile = `vs/code/electron-browser/workbench/workbench${
			this.environmentMainService.isBuilt ? "" : "-dev"
		}.html` as AppResourcePath;
		const loadUrl = FileAccess.asBrowserUri(htmlFile).toString(true);
		this._win.loadURL(loadUrl);
	...
	async reload(cli?: NativeParsedArgs): Promise<void> {
		...
		// Validate workspace
		configuration.workspace = await this.validateWorkspaceBeforeReload(
			configuration
		);
