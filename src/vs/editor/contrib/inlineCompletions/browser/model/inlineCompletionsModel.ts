...
export class InlineCompletionsModel extends Disposable {
			...
			(provider, store) => {
				...
				store.add(
					provider.onDidChangeInlineCompletions(() => {
						...
						if (
							activeState &&
							(activeState.inlineCompletion || activeState.edits) &&
							activeState.inlineCompletion?.source.provider !== provider
						) {
							return;
						}
		...
		(reader) => {
			...
			if (!this._shouldShowOnSuggestConflict.read(reader) && suggestItem) {
				...
				const validEditsAndGhostTexts = allPotentialEdits
					.map((edit, idx) => {
						if (!edit) {
							return { edit, ghostText: undefined };
						}

						// Calculate the remaining edit by removing the prefix that matches the model
						const remainingEdit = singleTextRemoveCommonPrefix(edit, model);
						if (remainingEdit.text.length === 0) {
							// If the text is empty, we still want to show the ghost text if it is not empty.
							// This can happen if the user has typed the prefix of the suggestion.
							// In this case, we want to show the ghost text for the next line.
							return { edit, ghostText: undefined };
						}

						// Truncate the remaining text to the first line for ghost text computation
						const lines = remainingEdit.text.split("\n");
						const firstLineText = lines[0] + (lines.length > 1 ? "\n" : "");
						const ghostTextEdit = new TextReplacement(
							remainingEdit.range,
							firstLineText
						);

						const ghostText = computeGhostText(
							ghostTextEdit,
							model,
							mode,
							positions[idx],
						...
						if (!ghostText) {
							// If no ghost text could be computed, it might be because the first line is empty or matches existing content.
							// We return the edit but no ghost text, effectively hiding it until more context is available.
							return { edit, ghostText: undefined };
						}
	...
	public readonly showCollapsed = derived<boolean>(this, (reader) => {
		...
		const isCurrentModelVersion =
			state.inlineCompletion.updatedEditModelVersion ===
	...
	public readonly tabShouldJumpToInlineEdit = derived(this, (reader) => {
		...
		if (
			...
			!s.inlineCompletion.hint?.jumpToEdit
		) {
			return false;
		}
	...
	public async accept(editor: ICodeEditor = this._editor): Promise<void> {
		...
		if (state?.kind === "ghostText") {
			...
			if (state.edits[0].text.includes("\n")) {
				await this.acceptNextLine();
				return;
			}
		...
		try {
			...
			if (isNextEditUri) {
				...
				const selections = getEndPositionsAfterApplying(minimalEdits).map((p) =>
	...
	public jump(): void {
		...
		transaction((tx) => {
			...
			const isSingleLineChange =
				...
				(s.inlineCompletion.hint ||
					!s.inlineCompletion.insertText.includes("\n"));
...
function isSuggestionInViewport(
	editor: ICodeEditor,
