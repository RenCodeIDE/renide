...
import {
	...
	type ICommandApprovalResult,
	type ICommandApprovalResultWithReason,
} from "../commandLineAutoApprover.js";
...
function createPowerShellModelDescription(shell: string): string {
	...
	return [
		`This tool allows you to execute ${
			isWinPwsh ? "Windows PowerShell 5.1" : "PowerShell"
		} commands in a persistent terminal session, preserving environment variables, working directory, and other context across multiple commands.`,
		"",
		"Command Execution:",
...
export class RunInTerminalTool extends Disposable implements IToolImpl {
	...
	protected readonly _sessionTerminalAssociations: Map<string, IToolTerminal> =
		new Map();

	// Immutable window state
	protected readonly _osBackend: Promise<OperatingSystem>;

	private static readonly _backgroundExecutions = new Map<
		string,
	...
	constructor(
		...
		this._terminalToolCreator =
			_instantiationService.createInstance(ToolTerminalCreator);
		this._treeSitterCommandParser = this._instantiationService.createInstance(
			TreeSitterCommandParser
		);
		this._commandSimplifier = _instantiationService.createInstance(
			CommandSimplifier,
			this._osBackend,
			this._treeSitterCommandParser
		);
		this._telemetry = _instantiationService.createInstance(
			RunInTerminalToolTelemetry
		);
		this._commandLineAutoApprover = this._register(
			_instantiationService.createInstance(CommandLineAutoApprover)
		);
		this._profileFetcher = _instantiationService.createInstance(
			TerminalProfileFetcher
		);
	...
	async prepareToolInvocation(
		...
		const presentation = alternativeRecommendation
		...
		const instance = context.chatSessionId
			? this._sessionTerminalAssociations.get(context.chatSessionId)?.instance
			: undefined;
		const terminalToolSessionId = generateUuid();

		let toolEditedCommand: string | undefined =
			await this._commandSimplifier.rewriteIfNeeded(args, instance, shell);
		...
		if (alternativeRecommendation) {
			...
			const isAutoApproveAllowed =
				isAutoApproveEnabled && isAutoApproveWarningAccepted;
			let isAutoApproved = false;

			let subCommands: string[] | undefined;
			const treeSitterLanguage = isPowerShell(shell, os)
			...
			try {
				...
				this._logService.info(
					`RunInTerminalTool: autoApprove: Parsed sub-commands via ${treeSitterLanguage} grammar`,
					subCommands
				);
			} catch (e) {
				console.error(e);
				this._logService.info(
					`RunInTerminalTool: autoApprove: Failed to parse sub-commands via ${treeSitterLanguage} grammar`
				);
			}

			if (subCommands) {
				const subCommandResults = subCommands.map((e) =>
					this._commandLineAutoApprover.isCommandAutoApproved(e, shell, os)
				);
				...
				if (deniedSubCommandResult) {
					this._logService.info(
						"RunInTerminalTool: autoApprove: Sub-command DENIED auto approval"
					);
					isDenied = true;
					...
					this._logService.info(
						"RunInTerminalTool: autoApprove: Command line DENIED auto approval"
					);
					isDenied = true;
					...
					if (subCommandResults.every((e) => e.result === "approved")) {
						this._logService.info(
							"RunInTerminalTool: autoApprove: All sub-commands auto-approved"
						);
						autoApproveReason = "subCommand";
						isAutoApproved = true;
						autoApproveDefault = subCommandResults.every(
							(e) => e.rule?.isDefaultRule
						);
					} else {
						this._logService.info(
							"RunInTerminalTool: autoApprove: All sub-commands NOT auto-approved"
						);
						if (commandLineResult.result === "approved") {
							this._logService.info(
								"RunInTerminalTool: autoApprove: Command line auto-approved"
							);
							autoApproveReason = "commandLine";
							isAutoApproved = true;
							autoApproveDefault = commandLineResult.rule?.isDefaultRule;
							...
							this._logService.info(
								"RunInTerminalTool: autoApprove: Command line NOT auto-approved"
							);
						}
					}
				}
			...
			confirmationMessages =
				isAutoApproved && isAutoApproveAllowed
					? undefined
					: {
							title: args.isBackground
								? localize(
										"runInTerminal.background",
										"Run `{0}` command? (background terminal)",
										shellType
								  )
	...
	async invoke(
		...
		const isNewSession =
			!args.isBackground &&
			!this._sessionTerminalAssociations.has(chatSessionId);
		...
		this._logService.debug(
			`RunInTerminalTool: Creating ${
				args.isBackground ? "background" : "foreground"
			} terminal. termId=${termId}, chatSessionId=${chatSessionId}`
		);
		const toolTerminal = await (args.isBackground
			? this._initBackgroundTerminal(
					chatSessionId,
					termId,
					terminalToolSessionId,
					token
			  )
			: this._initForegroundTerminal(
					chatSessionId,
					termId,
					terminalToolSessionId,
					token
		...
		if (args.isBackground) {
			...
			try {
				...
				store.dispose();
				this._logService.debug(
					`RunInTerminalTool: Finished polling \`${pollingResult?.output.length}\` lines of output in \`${pollingResult?.pollDurationMs}\``
				);
				...
				this._telemetry.logInvoke(toolTerminal.instance, {
					...
					outputLineCount: pollingResult?.output
						? count(pollingResult.output, "\n")
						: 0,
			...
			try {
				...
				switch (toolTerminal.shellIntegrationQuality) {
					case ShellIntegrationQuality.None: {
						strategy = this._instantiationService.createInstance(
							NoneExecuteStrategy,
							toolTerminal.instance,
				...
				store.add(
					strategy.onDidCreateStartMarker((startMarker) => {
						if (!outputMonitor) {
							outputMonitor = store.add(
								this._instantiationService.createInstance(
									OutputMonitor,
									{
										...
										getOutput: (marker?: IXtermMarker) =>
											getOutput(toolTerminal.instance, marker ?? startMarker),
									},
									undefined,
									invocation.context,
									token,
				...
				this._logService.debug(
					`RunInTerminalTool: Finished \`${strategy.type}\` execute strategy with exitCode \`${executeResult.exitCode}\`, result.length \`${executeResult.output?.length}\`, error \`${executeResult.error}\``
				);
				outputLineCount =
	...
	private async _initBackgroundTerminal(
	...
	private async _initForegroundTerminal(
		chatSessionId: string,
		termId: string,
		terminalToolSessionId: string | undefined,
		token: CancellationToken
		...
		if (cachedTerminal) {
			this._logService.debug(
				`RunInTerminalTool: Using cached foreground terminal with session ID \`${chatSessionId}\``
			);
	...
	private _registerInputListener(toolTerminal: IToolTerminal): void {
		...
		this._register(
			toolTerminal.instance.onDisposed(() => disposable.dispose())
		);
	}
	...
	private async _setupProcessIdAssociation(
		...
		this._register(
			toolTerminal.instance.onDisposed(() => {
	...
	private _cleanupSessionTerminals(sessionId: string): void {
		...
		if (toolTerminal) {
			// Check if this terminal has any active background executions
			const hasActiveBackgroundExecutions = Array.from(
				RunInTerminalTool._backgroundExecutions.values()
			).some((execution) => execution.instance === toolTerminal.instance);

			if (hasActiveBackgroundExecutions) {
				// Terminal has active background processes - keep it alive
				this._logService.debug(
					`RunInTerminalTool: Keeping terminal alive for disposed chat session ${sessionId} due to active background processes`
				);
				this._sessionTerminalAssociations.delete(sessionId);
				// Don't dispose the terminal or background executions
				// Background executions remain in the static map for get_terminal_output access
				return;
			}

			// No active background processes - safe to dispose
			this._logService.debug(
				`RunInTerminalTool: Cleaning up terminal for disposed chat session ${sessionId}`
			);
			...
			// (This should be rare since we check above, but handle edge cases)
			const terminalToRemove: string[] = [];
			for (const [
				termId,
				execution,
			] of RunInTerminalTool._backgroundExecutions.entries()) {
			...
			return asArray(result)
				.map((e) => {
					const settingsUri = createCommandUri(
						openTerminalSettingsLinkCommandId,
						e.rule!.sourceTarget
					);
					return `[\`${
						e.rule!.sourceText
					}\`](${settingsUri.toString()} "${localize(
						"ruleTooltip",
