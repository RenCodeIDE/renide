...
import { IRenViewManager, RenViewMode } from "./managers/renViewManager.js";
...
export class RenMainWindowOverlay {
	...
	private readonly _viewManager: IRenViewManager;
	private _isHandlingEvent = false;

	constructor(
		private readonly container: HTMLElement,
		@IContextKeyService private readonly contextKeyService: IContextKeyService,
		@IRenViewManager viewManager: IRenViewManager
	) {
		// tracks the current mode we are in
		this._currentMode = this.contextKeyService.createKey(
			"ren.currentViewMode",
			"code"
		);

		// Use injected view manager
	...
	private setupOverlay(): void {
		...
		console.log(
			"[RenMainWindowOverlay] Content area set, current view:",
			this._viewManager.getCurrentView()
		);
