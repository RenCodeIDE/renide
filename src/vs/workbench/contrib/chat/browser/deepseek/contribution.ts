	...
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		logService.info(
			`[deepseek-server] Environment check: SERVER_ADDRESS=${
		...
		if (!serverAddress) {
			logService.warn(
				"[deepseek-server] No server address configured. Set SERVER_ADDRESS environment variable or configure apiBaseUrl in product.json."
			);
			return;
		}
		...
		if (
			...
			logService.warn(
				`[deepseek-server] SERVER_ADDRESS missing protocol, assuming https://. Original: ${normalizedServerAddress}`
			);
		...
		logService.info(`[deepseek-server] Server address found: ${serverAddress}`);
		logService.info(
			`[deepseek-server] registering online agent using DeepSeek models`
		);
		...
		const registration = this.chatAgentService.registerAgent(agentId, {
			...
			description: localize(
				"deepseek.agent.description",
				"Use DeepSeek online models."
			),
			isCore: true,
			isDefault: false,
		...
		const provider: ILanguageModelChatProvider = {
			...
			async sendChatRequest(modelId, messages, _from, options, token) {
				...
				const modelToUse =
					...
					DEEPSEEK_MODELS.find((m) => m.isDefault)?.id ||
					"deepseek-chat";
				...
				try {
					...
					logServiceInstance.error(
						`[deepseek-provider] Error retrieving access token: ${
				...
				logServiceInstance.debug(
					`[deepseek-provider] Message format validation passed: ${messages.length} messages in IDE format`
				);

				const endpoint: "/api/agent/tools" = "/api/agent/tools";
				logServiceInstance.info(
					`[deepseek-provider] Using endpoint: ${endpoint} for model: ${modelToUse}`
				);
				...
				> {
					...
					if (!sawText && functionCallName && !token.isCancellationRequested) {
						yield {
							type: "text",
							value: localize(
								"deepseek.provider.functionCall",
								"DeepSeek wants to run tool {0}, but tools are only available in agent mode. Retry there or disable tool usage.",
								functionCallName
							),
						};
					}
...
registerWorkbenchContribution2(
	DeepSeekAgentContribution.ID,
	DeepSeekAgentContribution,
