...
export class LocalChatSessionsProvider
	extends Disposable
	implements IChatSessionItemProvider, IWorkbenchContribution
{
	...
	public get onDidChangeChatSessionItems() {
		return this._onDidChangeChatSessionItems.event;
	}
	...
	constructor(
		...
		this._register(
			this.chatSessionsService.registerChatSessionItemProvider(this)
		);
		...
		this._register(
			this.chatSessionsService.onDidChangeSessionItems((sessionType) => {
	...
	private registerWidgetListeners(): void {
			...
			this.chatWidgetService.onDidAddWidget((widget) => {
				...
				if (
					...
					widget.viewContext.viewId ===
						LocalChatSessionsProvider.CHAT_WIDGET_VIEW_ID
				) {
					this._onDidChange.fire();
						...
						widget.onDidChangeViewModel(() => {
							...
							if (widget.viewModel) {
								this.registerProgressListener(
									widget.viewModel.model.requestInProgressObs
								);
							}
						})
					);
		...
		const existingWidgets = this.chatWidgetService
	...
	private registerStatusListener(model: IChatModel): void {
		// Listen to status observable
		if ("sessionStatusObs" in model && model.sessionStatusObs) {
			const statusEvent = Event.fromObservableLight(model.sessionStatusObs);
			this._register(
				statusEvent(() => {
					this._onDidChangeChatSessionItems.fire();
				})
		...
		// Listen to recency observable
		if ("sessionRecencyObs" in model && model.sessionRecencyObs) {
			const recencyEvent = Event.fromObservableLight(model.sessionRecencyObs);
			this._register(
				recencyEvent(() => {
					this._onDidChangeChatSessionItems.fire();
				})
		...
		// Backward compatibility: listen to requestInProgressObs
	...
	private registerModelTitleListener(widget: IChatWidget): void {
		...
		if (model) {
			...
			this.registerStatusListener(model);
	...
	private getEditorKey(editor: EditorInput, group: IEditorGroup): string {
		return `${group.id}-${editor.typeId}-${
			editor.resource?.toString() || editor.getName()
		}`;
	}
	...
	private isLocalChatSession(editor?: EditorInput): boolean {
		...
		if (
			!isChatSession(
				this.chatSessionsService.getContentProviderSchemes(),
				editor
			)
		) {
			return false;
	...
	async provideChatSessionItems(
		...
		const chatWidget = this.chatWidgetService
			.getWidgetsByLocations(ChatAgentLocation.Chat)
			.find(
				(widget) =>
					typeof widget.viewContext === "object" &&
		...
		const status = chatWidget?.viewModel?.model
			? this.modelToStatus(chatWidget.viewModel.model)
			: undefined;
		const widgetSession: ChatSessionItemWithProvider = {
			...
			label:
				chatWidget?.viewModel?.model.title ||
				nls.localize2("chat.sessions.chatView", "Chat").value,
			description: nls.localize(
				"chat.sessions.chatView.description",
				"Chat View"
			),
			...
			timing: {
				startTime:
					chatWidget?.viewModel?.model.getRequests().at(0)?.timestamp || 0,
			},
			provider: this,
		};
		...
		this.editorOrder.forEach((editorKey, index) => {
			...
			if (editorInfo) {
				...
				if (
					editorInfo.editor instanceof ChatEditorInput &&
					editorInfo.editor.sessionId
				) {
