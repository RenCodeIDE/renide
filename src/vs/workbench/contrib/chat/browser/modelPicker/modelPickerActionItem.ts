		...
		getActions: () => {
			return delegate.getModels().map((model) => {
				return {
					...
					category:
						model.metadata.modelPickerCategory || DEFAULT_MODEL_PICKER_CATEGORY,
					class: undefined,
...
export class ModelPickerActionItem extends ActionWidgetDropdownActionViewItem {
	constructor(
		...
		widgetOptions:
			| Omit<IActionWidgetDropdownOptions, "label" | "labelRenderer">
			| undefined,
		delegate: IModelPickerDelegate,
		...
		const modelPickerActionWidgetOptions: Omit<
			...
			"label" | "labelRenderer"
		> = {
			actionProvider: modelDelegateToWidgetActionsProvider(
				delegate,
				telemetryService
			),
		...
		if (this.currentModel?.metadata.statusIcon) {
			domChildren.push(
				...renderLabelWithIcons(
					`\$(${this.currentModel.metadata.statusIcon.id})`
				)
			);
		}
		domChildren.push(
			dom.$(
				...
				undefined,
				this.currentModel?.metadata.name ??
					localize("chat.modelPicker.label", "Pick Model")
			)
		);
