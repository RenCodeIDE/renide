...
class ChatConfirmationNotifier extends Disposable {
	private readonly disposables = this._register(
	...
	async notify(
		...
		const notification = await dom.triggerNotification(
			titleText
				? localize("notificationTitle", "Chat: {0}", titleText)
				: localize("defaultTitle", "Chat: Confirmation Required"),
			{
				detail: localize(
					"notificationDetail",
					"The current chat session requires your confirmation to proceed."
				),
			}
		);
		if (notification) {
			...
			disposables.add(
				Event.once(notification.onClick)(async () => {
					await this._hostService.focus(targetWindow, {
						mode: FocusMode.Force,
					});
					const widget =
						this._chatWidgetService.getWidgetBySessionId(sessionId);
					if (widget) {
			...
			disposables.add(
				this._hostService.onDidChangeFocus((focus) => {
					if (focus) {
						disposables.dispose();
					}
				})
...
abstract class BaseSimpleChatConfirmationWidget<T> extends Disposable {
	private _onDidClick = this._register(
		new Emitter<IChatConfirmationButton<T>>()
	);
	...
	constructor(
		...
		this.notificationManager = this._register(
			instantiationService.createInstance(ChatConfirmationNotifier)
		);
		...
		if (
			...
			if (!targetWindow.document.hasFocus()) {
				this.notificationManager.notify(
					targetWindow,
					this.title,
					this.context.element.sessionId
				);
			}
		}
	}
...
abstract class BaseChatConfirmationWidget<T> extends Disposable {
	...
	protected renderMessage(
		...
		const isEmpty = typeof element === "string" && element.trim() === "";

		if (isEmpty) {
			// Hide message element when empty
			this.messageElement.style.display = "none";
			// Remove border from title when message is empty
			const titleElement = this._domNode.querySelector(
				".chat-confirmation-widget-title"
			);
			if (titleElement) {
				titleElement.classList.add("no-message");
			}
			return;
		}
		...
		if (titleElement) {
			titleElement.classList.remove("no-message");
		}

		if (!dom.isHTMLElement(element)) {
			const messageElement = this._register(
		...
		if (
			...
			if (!targetWindow.document.hasFocus()) {
				this.notificationManager.notify(
					targetWindow,
					this.title,
					this._context.element.sessionId
				);
			}
		}
	}
