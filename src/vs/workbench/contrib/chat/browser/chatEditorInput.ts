	...
	implements IEditorCloseHandler
{
		...
		if (!this.hasCustomTitle) {
			...
			this._register(
				toDisposable(() => {
					...
					if (!this.hasCustomTitle) {
						ChatEditorInput.countsInUseMap
							.get(this.inputName)
							?.delete(this.inputCount);
						if (
							ChatEditorInput.countsInUseMap.get(this.inputName)?.size === 0
						) {
	...
	override matches(otherInput: EditorInput | IUntypedEditorInput): boolean {
		...
		if (
			this.resource.scheme === Schemas.vscodeChatEditor &&
			otherInput.resource.scheme === Schemas.vscodeChatEditor
		) {
	...
	override getName(): string {
		...
		if (this.sessionId) {
			...
			const persistedTitle = this.chatService.getPersistedSessionTitle(
				this.sessionId
			);
	...
	override getTitle(verbosity?: Verbosity): string {
		...
		if (verbosity === Verbosity.LONG) {
			...
			const sessionTypeDisplayName =
				this.getSessionTypeDisplayName(sessionType);
	...
	private resolveIcon(): ThemeIcon | URI | undefined {
		...
		if (sessionType !== "local") {
			const typeIcon =
				this.chatSessionsService.getIconForSessionType(sessionType);
			if (typeIcon) {
				return typeIcon;
			}
		}
	...
	public getSessionType(): string {
		if (
			this.resource.scheme === Schemas.vscodeChatEditor ||
			this.resource.scheme === Schemas.vscodeChatSession
		) {
			return "local";
		}
	...
	override async resolve(): Promise<ChatEditorModel | null> {
		...
		if (this.resource.scheme !== Schemas.vscodeChatEditor) {
			this.model = await this.chatService.loadSessionForResource(
				this.resource,
			...
			this.model =
				...
				this.chatService.startSession(
					...
					CancellationToken.None,
					undefined,
					{ canUseTools: false, inputType: inputType }
				);
			...
			this.model = this.chatService.startSession(
				...
				CancellationToken.None,
				undefined,
				{ canUseTools: !inputType, inputType: inputType }
			);
