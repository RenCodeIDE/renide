...
} from "../common/chatTodoListService.js";
import {
	PlanExecutionTracker,
	IPlanExecutionTracker,
} from "../common/planExecutionTracker.js";
import {
	ChatTransferService,
...
import {
	...
	OpenChatSessionInNewEditorGroupAction,
	OpenChatSessionInNewWindowAction,
	OpenChatSessionInSidebarAction,
	RenameChatSessionAction,
	ToggleChatSessionsDescriptionDisplayAction,
} from "./actions/chatSessionActions.js";
...
} from "./chatAttachmentResolveService.js";
import {
...
} from "./promptSyntax/saveToPromptAction.js";
import {
	ConfigureToolSets,
	...
	properties: {
		...
		[ChatConfiguration.GlobalAutoApprove]: {
			...
			policy: {
				...
				minimumVersion: "1.99",
				value: (account) =>
					account.chat_preview_features_enabled === false ? false : undefined,
				localization: {
					description: {
						key: "autoApprove2.description",
						value: nls.localize(
							"autoApprove2.description",
		...
		[ChatConfiguration.ToolPermissionPreference]: {
			...
			enumDescriptions: [
				nls.localize(
					"toolPermissionPreference.ask",
					"Ask for permission before running each tool"
				),
				nls.localize(
					"toolPermissionPreference.always",
					"Always allow tools to run without asking"
				),
			],
			default: "ask",
			markdownDescription: nls.localize(
				"toolPermissionPreference.description",
				"Controls whether the IDE should ask for permission before running each tool, or always allow tools to run automatically. Set to `ask` to be prompted each time, or `always` to automatically approve all tool executions."
			),
			scope: ConfigurationScope.APPLICATION,
		},
		[ChatConfiguration.AutoApproveEdits]: {
			default: {
				...
				"**/.vscode/*.json": false,
				"**/.git/**": false,
				"**/{package.json,package-lock.json,server.xml,build.rs,web.config,.gitattributes,.env}":
					false,
		...
		[ChatConfiguration.EmptyStateHistoryEnabled]: {
			...
			description: nls.localize(
				...
				"Show recent chat history on the empty chat state."
			),
			tags: ["experimental"],
		},
		...
		[ChatConfiguration.AgentEnabled]: {
			...
			policy: {
				...
				minimumVersion: "1.99",
				value: (account) =>
					account.chat_agent_enabled === false ? false : undefined,
				localization: {
					description: {
		...
		[ChatConfiguration.AgentSessionsViewLocation]: {
			...
			description: nls.localize(
				"chat.sessionsViewLocation.description",
		...
		[ChatConfiguration.ThinkingStyle]: {
			...
			enumDescriptions: [
				nls.localize(
					"chat.agent.thinkingMode.default",
					"Let VS Code choose a thinking style for each model."
				),
				nls.localize(
				...
				nls.localize(
					"chat.agent.thinkingMode.expanded",
					"Thinking parts will be expanded by default."
				),
				nls.localize(
					"chat.agent.thinkingMode.none",
					"Do not show the thinking"
				),
				nls.localize(
					"chat.agent.thinkingMode.collapsedPerItem",
				...
				nls.localize(
					"chat.agent.thinkingMode.fixedScrollingTools",
					"Show thinking and certain tool calls in a fixed-height streaming panel that auto-scrolls; click header to expand to full height."
				),
			],
			description: nls.localize(
		...
		"chat.agent.thinking.collapsedTools": {
			...
			description: nls.localize(
				"chat.agent.thinking.collapsedTools",
				"When enabled, tool calls will be added by default inside the thinking block."
			),
			tags: ["experimental"],
		},
		...
		[ChatConfiguration.ShowAgentSessionsViewDescription]: {
			type: "boolean",
			description: nls.localize(
				"chat.showAgentSessionsViewDescription",
		...
		"copilot.extensionUnification.enabled": {
			...
			description: nls.localize(
				"copilot.extensionUnification.enabled",
				"Enables unification of GitHub Copilot extensions. When enabled, only the GitHub Copilot Chat extension will be active and all functionality will be served by it. When disabled, GitHub Copilot Chat and GitHub Copilot extensions will operate independently."
			),
			default: false,
			tags: ["experimental"],
			experiment: {
							...
							agents
								...
								.map(async (a) => {
									...
									const agentMarkdown = instantiationService.invokeFunction(
										(accessor) => agentToMarkdown(a, true, accessor)
									);
									...
									const commandText = a.slashCommands
										.map((c) => {
											const description = c.description
												? `- ${c.description}`
												: "";
											return `\t* ${agentSlashCommandToMarkdown(
												a,
												c
											)} ${description}`;
										})
...
registerWorkbenchContribution2(
	LocalChatSessionsProvider.ID,
	LocalChatSessionsProvider,
	WorkbenchPhase.AfterRestored
);
registerWorkbenchContribution2(
	ChatSessionsViewContrib.ID,
	ChatSessionsViewContrib,
	WorkbenchPhase.AfterRestored
);
...
registerWorkbenchContribution2(
	PlanFilePreviewHandler.ID,
	PlanFilePreviewHandler,
	WorkbenchPhase.AfterRestored
);
...
registerWorkbenchContribution2(
	ChatAgentNotificationContribution.ID,
	ChatAgentNotificationContribution,
	WorkbenchPhase.Eventually
);
