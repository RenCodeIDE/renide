	...
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		logService.info(
			`[chatgpt-server] Environment check: SERVER_ADDRESS=${
		...
		if (!serverAddress) {
			logService.warn(
				"[chatgpt-server] No server address configured. Set SERVER_ADDRESS environment variable or configure apiBaseUrl in product.json."
			);
			logService.info(
		...
		try {
			agentPlanner = this.instantiationService.invokeFunction((accessor) =>
				accessor.get(IAgentPlanner)
			);
		} catch {
		...
		try {
			dependencyGraphService = this.instantiationService.invokeFunction(
				(accessor) => accessor.get(IDependencyGraphService)
			);
		} catch {
			// Service not available
		}
		...
		try {
			workspaceService = this.instantiationService.invokeFunction((accessor) =>
		...
		try {
			fileService = this.instantiationService.invokeFunction((accessor) =>
		...
		const provider: ILanguageModelChatProvider = {
			...
			async provideLanguageModelChatInfo(
				...
				try {
					...
					logService.info(
						`[chatgpt] provideLanguageModelChatInfo returning ${
							models.length
						} models: ${models.map((m) => m.identifier).join(", ")}`
					);
					return models;
				} catch (error) {
					logService.error(
			...
			async sendChatRequest(modelId, messages, _from, _options, token) {
				throw new Error(
					"OpenAI models must run in agent mode. Use ChatGPT agent."
				);
			},
