...
export class ChatViewPane extends ViewPane implements IViewWelcomeDelegate {
	...
	constructor(
		...
		if (
			this.chatOptions.location === ChatAgentLocation.Chat &&
			!this.viewState.hasMigratedCurrentSession
		) {
			const editsMemento = new Memento<IViewPaneState>(
	...
	override shouldShowWelcome(): boolean {
		...
		this.logService.trace(
			`ChatViewPane#shouldShowWelcome(${
				this.chatOptions.location
			}) = ${shouldShow}: hasCoreAgent=${hasCoreAgent} hasDefaultAgent=${hasDefaultAgent} || noViewModel=${!this
				._widget?.viewModel} && noPersistedSessions=${noPersistedSessions}`
		);
		return !!shouldShow;
	}
	...
	private async clear(): Promise<void> {
		if (this.widget.viewModel) {
			const sessionId = this.widget.viewModel.sessionId;
			// Check if session has pending requests
			const hasPendingRequests = this.chatService.hasPendingRequest(sessionId);

			if (hasPendingRequests) {
				// Detach instead of clear - keeps execution running
				await this.chatService.detachSession(sessionId);
			} else {
	...
	async loadSession(
		sessionId: string | URI,
		viewState?: IChatViewState
	): Promise<void> {
		if (this.widget.viewModel) {
			const currentSessionId = this.widget.viewModel.sessionId;
			const hasPendingRequests =
				this.chatService.hasPendingRequest(currentSessionId);
		...
		if (
			URI.isUri(sessionId) &&
			sessionId.scheme === Schemas.vscodeChatSession
		) {
			...
			if (parsed?.chatSessionType) {
				...
				const contribution = contributions.find(
					(c: IChatSessionsExtensionPoint) => c.type === parsed.chatSessionType
				);
				if (contribution) {
		...
		const newModel = await (URI.isUri(sessionId)
			? this.chatService.loadSessionForResource(
					sessionId,
