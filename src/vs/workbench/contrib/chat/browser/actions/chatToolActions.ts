...
class ConfigureToolsActionRendering implements IWorkbenchContribution {
	...
	constructor(
		...
		const disposable = actionViewItemService.register(
			...
			(action, _opts, instantiationService) => {
				...
				return instantiationService.createInstance(
					class extends MenuEntryActionViewItem {
						...
						override render(container: HTMLElement): void {
							...
							this.warningElement = $(
								`.tool-warning-indicator${ThemeIcon.asCSSSelector(
									Codicon.warning
								)}`
							);
						...
						private isAboveToolLimit() {
							const rawToolLimit = this._contextKeyService.getContextKeyValue(
								ChatContextKeys.chatToolGroupingThreshold.key
							);
							const rawToolCount = this._contextKeyService.getContextKeyValue(
								ChatContextKeys.chatToolCount.key
							);
