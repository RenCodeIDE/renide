		...
		class ChatHistoryAction extends Action2 {
				...
				const getPicks = async () => {
					...
					for (const item of items) {
						if (item.hasPendingRequest) {
							running.push(item);
						} else if (isToday(item.lastMessageDate ?? 0)) {
							recent.push(item);
						} else {
							old.push(item);
						}
					...
					// Build picks with separators
					const picks: (IQuickPickSeparator | IChatPickerItem)[] = [];

					// Running section
					if (running.length > 0) {
						picks.push({
							type: "separator",
							label: localize("chat.history.running", "Running"),
						});
						for (const i of running) {
							picks.push({
								label: `$(sync~spin) ${i.title}`,
					...
					if (recent.length > 0) {
						picks.push({
							type: "separator",
							label: localize("chat.history.recent", "Recent"),
						});
					...
					if (old.length > 0) {
						picks.push({
							type: "separator",
							label: localize("chat.history.old", "Older"),
						});
						for (const i of old) {
			...
			private async showIntegratedPicker(
				...
				const getPicks = async (
					...
					// Build fast picks with sections
					...
					if (running.length > 0) {
						...
						const runningItems = showAllChats ? running : running.slice(0, 5);
						for (const i of runningItems) {
							fastPicks.push({
								label: `$(sync~spin) ${i.title}`,
								description: i.isActive
									? `(${localize("currentChatLabel", "current")})`
					...
					if (recent.length > 0) {
						...
						const recentItems = showAllChats ? recent : recent.slice(0, 5);
					...
					const totalItems = running.length + recent.length + old.length;
					const shownItems =
						running.slice(0, 5).length + recent.slice(0, 5).length;
					if (!showAllChats && totalItems > shownItems) {
						fastPicks.push({
						...
						try {
							...
							try {
								const providerNSessions =
									await chatSessionsService.getAllChatSessionItems(
								...
								for (const { chatSessionType, items } of providerNSessions) {
									for (const session of items) {
										...
										const { primary } = getContextMenuActions(
											actions,
											"inline"
										);
										ckey.reset();
										...
										const existingIndex = agentPicks.findIndex(
											(pick) => pick.chat.sessionId === session.id
										);
										if (existingIndex >= 0) {
				...
				super({
					...
					menu: {
						...
						when: ContextKeyExpr.equals(
							"view",
							`${AGENT_SESSIONS_VIEWLET_ID}.local`
						),
					},
				});
			}
...
const providerConfig: ProductProviderConfig =
	product.defaultChatAgent?.provider ?? {};
const defaultChat = {
	...
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		const disposable = actionViewItemService.register(
			...
			(action, options, instantiationService, windowId) => {
				...
				let primaryActionId = isAuxiliaryWindow
					? CHAT_OPEN_ACTION_ID
					: TOGGLE_CHAT_ACTION_ID;
				let primaryActionTitle = isAuxiliaryWindow
					? localize("openChat", "Open Chat")
...
registerAction2(
	class ToggleToolPermissionPreference extends Action2 {
		constructor() {
			super({
				id: "workbench.action.chat.toggleToolPermissionPreference",
				title: localize2(
					"chat.toggleToolPermissionPreference.label",
					"Toggle Always Allow Tools"
				),
				f1: true,
		...
		async run(accessor: ServicesAccessor): Promise<void> {
			...
			const currentValue = configurationService.getValue<string>(
			...
			const message =
				newValue === "always"
					? localize(
							"chat.toggleToolPermissionPreference.enabled",
							"Always Allow Tools is now enabled. All tools will run automatically without asking for confirmation."
					  )
					: localize(
							"chat.toggleToolPermissionPreference.disabled",
							"Always Allow Tools is now disabled. You will be asked for permission before each tool runs."
					  );
...
registerAction2(
	class ToggleChatHistoryVisibilityAction extends Action2 {
		constructor() {
			super({
				id: "workbench.action.chat.toggleChatHistoryVisibility",
				title: localize2(
					"chat.toggleChatHistoryVisibility.label",
					"Chat History"
				),
				category: CHAT_CATEGORY,
				precondition: ChatContextKeys.enabled,
...
registerAction2(
	class OpenChatEmptyStateSettingsAction extends Action2 {
		constructor() {
			super({
				id: "workbench.action.chat.openChatEmptyStateSettings",
				title: localize2(
					"chat.openChatEmptyStateSettings.label",
