...
} from "../../../remoteCodingAgents/common/remoteCodingAgentsService.js";
import {
	IChatAgent,
...
export class ChatDelegateToEditSessionAction extends Action2 {
	...
	constructor() {
		super({
			...
			title: localize2(
				"interactive.submit.panel.label",
	...
	override async run(
		...
		if (
			inlineWidget &&
			locationData?.type === ChatAgentLocation.EditorInline &&
			locationData.delegateSessionId
		) {
			const sessionWidget = widgetService.getWidgetBySessionId(
				locationData.delegateSessionId
			);
...
type ChatModeChangeClassification = {
	...
	fromMode?: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "The previous agent";
	};
	toMode?: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
		comment: "The new agent";
	};
	requestCount?: {
		classification: "SystemMetaData";
		purpose: "FeatureInsight";
...
class ToggleChatModeAction extends Action2 {
	...
	async run(accessor: ServicesAccessor, ...args: unknown[]) {
		...
		const switchToMode =
			(arg && modeService.findModeById(arg.modeId)) ??
			this.getNextMode(
				context.chatWidget,
				requestCount,
		...
		const chatModeCheck = await instaService.invokeFunction(
			handleModeSwitch,
			context.chatWidget.input.currentModeKind,
			switchToMode.kind,
			requestCount,
...
export class CreateRemoteAgentJobAction extends Action2 {
	...
	constructor() {
		...
		super({
			...
			menu: [], // Removed menu registration to hide cloud paper plane icon
		});
	}

	private async pickCodingAgent<
		T extends IChatSessionsExtensionPoint | IRemoteCodingAgent
	>(
		...
		const pick = await quickPickService.pick(
			...
			})),
			{
				placeHolder: localize(
					...
					"Select Agent to delegate the task to"
				),
			}
		);
		if (!pick) {
	...
	private async createWithLegacy(
		...
		const result: Omit<IChatPullRequestContent, "kind"> | string | undefined =
	 ...
	 */
	private extractRelativeFromAttachedContext(
		attachedContext: ChatRequestVariableSet,
		...
		for (const contextEntry of attachedContext.asArray()) {
			if (isChatRequestFileEntry(contextEntry)) {
				// TODO: Extend for more variable types as needed
				if (!(contextEntry.value instanceof URI)) {
					continue;
				}
				const workspaceFolder = workspaceContextService.getWorkspaceFolder(
					contextEntry.value
				);
				const fileUri = contextEntry.value;
				const relativePathResult = workspaceFolder
					? relativePath(workspaceFolder.uri, fileUri)
					: undefined;
	...
	async run(accessor: ServicesAccessor, ...args: unknown[]) {
		...
		try {
			...
			const attachedContext =
				widget.input.getAttachedAndImplicitContext(sessionId);
			...
			const agent = await this.pickCodingAgent(
				quickPickService,
				sortedContributions
			);
			if (!agent) {
			...
			const parsedRequest = requestParser.parseChatRequest(
				sessionId,
				userPrompt,
				ChatAgentLocation.Chat
			);
			...
			if (defaultAgent && userPrompt.length > 10_000) {
				chatModel.acceptResponseProgress(addedRequest, {
					...
					content: new MarkdownString(
						localize(
							"summarizeUserPromptCreateRemoteJob",
							"Summarizing user prompt"
						),
						CreateRemoteAgentJobAction.markdownStringTrustedOptions
					),
				});

				({ title, summarizedUserPrompt } =
					await this.generateSummarizedUserPrompt(
						sessionId,
						userPrompt,
						attachedContext,
						title,
						chatAgentService,
						defaultAgent,
						summarizedUserPrompt
					));
			}

			let summary: string = "";
			const relativeAttachedContext = this.extractRelativeFromAttachedContext(
				attachedContext,
			...
			attachedContext.asArray().forEach((ctx) => {
				if (isChatRequestFileEntry(ctx) && ctx.value && isLocation(ctx.value)) {
					...
					if (workspaceFolder && ctx.value.uri) {
						const relativePathResult = relativePath(
							workspaceFolder.uri,
							ctx.value.uri
						);
						if (relativePathResult) {
			...
			if (defaultAgent && chatRequests.length > 1) {
				...
				({ title, summary } = await this.generateSummarizedChatHistory(
					chatRequests,
					sessionId,
					title,
					chatAgentService,
			...
			await this.createWithLegacy(
				remoteCodingAgentService,
				commandService,
				quickPickService,
				chatModel,
				addedRequest,
				widget,
				summarizedUserPrompt || userPrompt,
				summary
			);
	...
	private async generateSummarizedChatHistory(
		chatRequests: IChatRequestModel[],
		sessionId: string,
		...
		const historyEntries: IChatAgentHistoryEntry[] = chatRequests.map(
			(req) => ({
				request: {
					sessionId: sessionId,
					requestId: req.id,
		...
		title ??= await chatAgentService.getChatTitle(
			defaultAgent.id,
			historyEntries,
			CancellationToken.None
		);
		summary += await chatAgentService.getChatSummary(
			defaultAgent.id,
			historyEntries,
			CancellationToken.None
		);
	...
	private async generateSummarizedUserPrompt(
		sessionId: string,
		userPrompt: string,
		attachedContext: ChatRequestVariableSet,
		title: string | undefined,
		chatAgentService: IChatAgentService,
		defaultAgent: IChatAgent,
		summarizedUserPrompt: string | undefined
	) {
		...
		summarizedUserPrompt = await chatAgentService.getChatSummary(
			defaultAgent.id,
			historyEntries,
			CancellationToken.None
		);
