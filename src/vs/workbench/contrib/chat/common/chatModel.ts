...
export interface IChatRequestModel {
	...
	readonly replyTo?: IChatReplyMetadata;
}
...
export class ChatRequestModel implements IChatRequestModel {
	...
	private readonly _replyTo?: IChatReplyMetadata;
	...
	public get replyTo(): IChatReplyMetadata | undefined {
...
class ResponseView extends AbstractResponse {
	constructor(_response: IResponse, public readonly undoStop: string) {
...
export class ChatModel extends Disposable implements IChatModel {
	...
	readonly sessionStatusObs: IObservable<ChatSessionStatus>;
	readonly sessionRecencyObs: IObservable<ChatSessionRecency>;
	private readonly _sessionStatusObs = observableValue<ChatSessionStatus>(
		this,
		ChatSessionStatus.Completed
	);
	private readonly _sessionRecencyObs = observableValue<ChatSessionRecency>(
		this,
	...
	constructor(
		...
		if (initialData && !isValid) {
			this.logService.warn(
				`ChatModel#constructor: Loaded malformed session data: ${JSON.stringify(
					initialData
				)}`
			);
		}
		...
		if (isValid && initialData && "lastActivityTime" in initialData) {
			...
			this._lastActivityTime =
				initialData.lastActivityTime ?? this._creationDate;
		} else {
	...
	private _updateSessionStatus(): void {
		...
		if (
			...
			this._lastAgentResponseEndTime > now
		) {
			...
			if (requests.length === 0) {
				...
				if (lastRequest?.response) {
					...
					if (
						...
						const endTime =
							this._lastAgentResponseEndTime ?? lastRequest.timestamp;
						...
						this._lastActivityTime = Math.max(lastRequest.timestamp, endTime);
					}
				} else {
			...
			requests: this._requests.map((r): ISerializableChatRequestData => {
				...
				return {
					...
					responseMarkdownInfo:
						r.response?.codeBlockInfos?.map<ISerializableMarkdownInfo>(
							(info) => ({ suggestionId: info.suggestionId })
						),
...
export namespace ChatResponseResource {
	...
	export function parseUri(
		uri: URI
	): undefined | { sessionId: string; toolCallId: string; index: number } {
