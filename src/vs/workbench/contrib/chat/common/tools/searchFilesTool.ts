...
export const SearchFilesToolData: IToolData = {
	...
	modelDescription: localize(
		"searchFilesTool.modelDescription",
...
- "Find the login handler" → search for "login" with includePattern:"*.ts"`
		...
		properties: {
			pattern: {
				...
				description: localize(
					"searchFilesTool.pattern",
					"The text pattern to search for. Can be a plain string or regular expression if useRegex is true."
				),
			},
			folder: {
				type: "string",
				description: localize(
					"searchFilesTool.folder",
					"Optional: The workspace folder to search in. If not specified, searches all workspace folders."
				),
			},
			includePattern: {
				type: "string",
				description: localize(
					"searchFilesTool.includePattern",
					'Optional: File glob patterns to include (e.g., "*.ts", "**/*.js"). Filenames without wildcards will use fuzzy matching (e.g., "config" matches "config.json", "config.yaml"). Multiple patterns can be separated by commas.'
				),
			},
			excludePattern: {
				type: "string",
				description: localize(
					"searchFilesTool.excludePattern",
					'Optional: File glob patterns to exclude (e.g., "node_modules/**", "*.min.js"). Multiple patterns can be separated by commas.'
				),
			},
			caseSensitive: {
				type: "boolean",
				description: localize(
					"searchFilesTool.caseSensitive",
			...
			useRegex: {
				type: "boolean",
				description: localize(
					"searchFilesTool.useRegex",
					"Optional: Whether to treat the pattern as a regular expression. Defaults to false."
				),
			},
			maxResults: {
				type: "number",
				description: localize(
					"searchFilesTool.maxResults",
					"Optional: Maximum number of results to return. Defaults to 1000."
				),
			},
		},
		required: ["pattern"],
	...
	resolveDefaults: (ctx) => {
		...
		if (
			ctx.selectionText &&
			ctx.selectionText.trim().length > 0 &&
			ctx.selectionText.length < 100
		) {
	...
	async prepareToolInvocation(
		...
		return {
			invocationMessage: localize(
				"searchFilesTool.invocationMessage",
				"Searching for: {0}",
				args.pattern
			),
			pastTenseMessage: localize(
				"searchFilesTool.pastTenseMessage",
				"Searched for: {0}",
				args.pattern
			),
		};
				...
				content: [
					{
						kind: "text",
						value: localize(
							"searchFilesTool.invalidPattern",
...
ALTERNATIVE: If you cannot determine what to search for, ask the user what text or code they want to find.`
				...
				toolResultMessage: localize(
					"searchFilesTool.invalidPatternShort",
					'Missing required "pattern" argument. Please provide a search pattern.'
				),
			};
		}
		...
		try {
			...
			if (args.folder) {
				...
				const folderInWorkspace = folders.find(
					(f) =>
						folderUri.toString().startsWith(f.toString()) ||
						folderUri.toString() === f.toString()
				);
					...
					content: [
						{
							kind: "text",
							value: localize(
								"searchFilesTool.noWorkspace",
								"No workspace folder found to search in."
							),
						},
					],
					toolResultMessage: localize(
			...
			await this.searchService.textSearch(
				query,
				token,
				(progress: ISearchProgressItem) => {
								...
								if (resultIsMatch(result)) {
									...
									if (match.rangeLocations && match.rangeLocations.length > 0) {
										for (const rangeLocation of match.rangeLocations) {
											...
											results.push(
												`  ${lineNumber}:${column} ${preview.trim()}`
											);
										}
									}
								}
					...
					content: [
						{
							kind: "text",
							value: localize(
								"searchFilesTool.noResults",
								"No results found for pattern: {0}",
								args.pattern
							),
						},
					],
			...
			const summary = localize(
				"searchFilesTool.summary",
				'Found {0} result(s) for pattern "{1}"',
				resultCount,
				args.pattern
			);
				...
				content: [
					{
						kind: "text",
						value: localize(
							"searchFilesTool.error",
							"Error searching files: {0}",
							errorMessage
						),
					},
				],
