...
import {
	CreatePlanFileTool,
	CreatePlanFileToolData,
} from "./createPlanFileTool.js";
	...
	implements IWorkbenchContribution
{
	...
	constructor(
		...
		this._register(
			toolsService.registerTool(ManagePlanToolData, managePlanTool)
		);

		// Register the plan file tools (Plan Mode)
		const createPlanFileTool =
			instantiationService.createInstance(CreatePlanFileTool);
		this._register(
			toolsService.registerTool(CreatePlanFileToolData, createPlanFileTool)
		);
		...
		this._register(
			toolsService.registerTool(CreateFileToolData, createFileTool)
		);

		const deleteFileTool = instantiationService.createInstance(DeleteFileTool);
		this._register(
			toolsService.registerTool(DeleteFileToolData, deleteFileTool)
		);

		const searchFilesTool =
			instantiationService.createInstance(SearchFilesTool);
		this._register(
			toolsService.registerTool(SearchFilesToolData, searchFilesTool)
		);
		...
		const checkLinterTool =
			instantiationService.createInstance(CheckLinterTool);
		this._register(
			toolsService.registerTool(CheckLinterToolData, checkLinterTool)
		);

		// Register the semantic codebase search tool
		const semanticSearchTool =
			instantiationService.createInstance(SemanticSearchTool);
		this._register(
			toolsService.registerTool(SemanticSearchToolData, semanticSearchTool)
		);
	}
}
