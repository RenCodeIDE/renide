...
export const GraphControlToolData: IToolData = {
	...
	modelDescription: localize(
		"graphControlTool.modelDescription",
		"Controls the graph view to display different types of visualizations. Can change graph type (file dependencies, folder structure, workspace overview, git heatmap, data flow analysis, evolution timeline, or change impact) and optionally specify a target file or folder to visualize. Automatically switches to graph view if not already active."
	),
		...
		properties: {
			graphType: {
				...
				description: localize(
					"graphControlTool.graphType",
					'The type of graph to display: "file" for file dependencies, "folder" for folder dependencies, "workspace" for workspace overview, "gitHeatmap" for git commit coupling, "dataFlow" for function call flow, "evolution" for timeline view, "changeImpact" for change impact analysis'
				),
			},
			targetPath: {
				type: "string",
				description: localize(
					"graphControlTool.targetPath",
					'Optional: The file or folder path to visualize. Required for "file" and "folder" modes. Can be absolute or workspace-relative path.'
				),
			},
		},
		required: [],
...
export class GraphControlTool implements IToolImpl {
	...
	async prepareToolInvocation(
		...
		const targetInfo = parameters.targetPath
			? ` for ${parameters.targetPath}`
			: "";

		return {
			invocationMessage: localize(
				"graphControlTool.invocationMessage",
				"Loading {0}{1}...",
				graphTypeLabel,
				targetInfo
			),
			pastTenseMessage: localize(
				"graphControlTool.pastTenseMessage",
				"Loaded {0}{1}",
				graphTypeLabel,
				targetInfo
			),
	...
	async invoke(
		...
		const graphType = (args.graphType || "workspace")
			.trim()
			.toLowerCase() as GraphMode;

		// Validate graph type
		const validGraphTypes: GraphMode[] = [
			"file",
			"folder",
			"workspace",
			"architecture",
				...
				content: [
					{
						kind: "text",
						value: localize(
							"graphControlTool.invalidType",
							'Invalid graph type "{0}". Must be one of: {1}',
							graphType,
							validGraphTypes.join(", ")
						),
					},
				],
				toolResultMessage: localize(
					"graphControlTool.error",
					'Failed to control graph: invalid type "{0}". Must be one of: {1}',
					graphType,
					validGraphTypes.join(", ")
				),
			};
		...
		if ((graphType === "file" || graphType === "folder") && !args.targetPath) {
			return {
				content: [
					{
						kind: "text",
						value: localize(
							"graphControlTool.missingTarget",
							'Graph type "{0}" requires a targetPath parameter',
							graphType
						),
					},
				],
				toolResultMessage: localize(
					"graphControlTool.error",
					"Failed to control graph: missing target path"
				),
			};
		}
		...
		try {
			...
			const message = localize(
				"graphControlTool.success",
				"Graph view showing {0}{1}",
				graphTypeLabel,
				targetInfo
			);
	...
	private getGraphTypeLabel(graphType: GraphMode): string {
		switch (graphType) {
			...
			case "folder":
				return localize(
					"graphControlTool.folderGraph",
					"Folder Dependency Graph"
				);
			case "workspace":
				return localize(
					"graphControlTool.workspaceGraph",
					"Workspace Overview Graph"
				);
			case "gitHeatmap":
				return localize(
					"graphControlTool.gitHeatmap",
					"Git Commit Coupling Heatmap"
				);
			case "dataFlow":
				return localize("graphControlTool.dataFlow", "Data Flow Graph");
			case "architecture":
				return localize(
					"graphControlTool.architecture",
